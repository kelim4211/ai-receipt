...
ITEM: 원본제품명 | 정밀복원제품명(제품검색에 최적화된 공식 명칭) | 단가곱하기수량의합(가장오른쪽총액숫자) | 할인액 | 최종금액
ETC: 항목명 | 금액
...
[제품검색 키워드 추출 2대 분기 원칙]
1. 편의점 및 일반 소매점 상호 배제 원칙:
   - 영수증 상호가 편의점(CU, GS25 등), 일반 소매점, 마트, 식당 등의 개별 가맹점/지점명인 경우, 복원제품명(두 번째 필드)에 해당 상호나 지점명을 절대 포함시키지 마십시오. 오직 상품 본연의 명칭과 제조사/브랜드 위주로만 구성하십시오.
2. 유통사 브랜드(다이소 등) 한정 허용 및 포함 원칙:
   - 영수증 상호가 '다이소' 등 자체 브랜드 유통사로 판별되거나 '다이소 (추정)'으로 유추된 경우, 제품 검색 정확도를 위해 해당 상호명('다이소')을 복원제품명(두 번째 필드)의 맨 앞에 반드시 포함시켜 조합하십시오 (예: 다이소 PVC 커버 접착식 앨범).

[순수 상품명 추출 및 규격/수량 제거 절대 규칙]
- 복원제품명(두 번째 필드)은 영수증에 적힌 축약어, 불필요한 괄호, 기호(예: ')')를 제거하고, 실제 인터넷 쇼핑이나 제품 검색 시 가장 정확하게 매칭되는 정식 명칭으로 구성하십시오.
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        contents: [
          {
            parts: [
              { text: "영수증을 분석하되, 편의점 지점명은 제품검색 키워드에서 철저히 배제하고, 다이소 같은 유통사 브랜드는 제품검색 키워드에 포함시키는 분기 원칙을 적용하여 정식 제품명을 복원하고, 최종 결제 총액(TOTAL)을 정확히 분석하여 지정된 양식으로 출력하시오." },
