...
- 할인 항목(-)이나 증정, 쿠폰 등의 복잡한 텍스트가 있더라도 유효한 JSON 문법(따옴표, 쉼표, 괄호 짝)을 철저히 준수하십시오.
- 각 품목별로 포털 검색 적중률을 높일 수 있는 최적의 맞춤 검색어('searchQuery')를 포함하십시오.`;
    ...
    const promptText = `영수증 이미지를 분석하여 아래 포맷의 순수 JSON으로만 응답하시오:
{
  ...
  "products": [
    ...
    try {
      ...
      console.error('API Response Parse Error:', responseText);
    ...
    try {
      ...
      console.warn('1차 JSON 파싱 실패, 자동 교정 및 정제(Sanitation) 시도...', rawJsonText);
      
      try {
        // 2차 자가 교정 정제 (제어문자 제거, 잘못된 마지막 쉼표 제거, 따옴표 누락 보완 등)
        ...
        console.error('JSON Sanitation Failed:', innerErr);
