...
ITEM: 원본제품명 | 순수복원제품명(규격/수량제외한_순수상품명) | 단가곱하기수량의합(가장오른쪽총액숫자) | 할인액 | 최종금액
ETC: 항목명 | 금액
...
[순수 상품명 추출 규칙]
- ITEM의 두 번째 필드(순수복원제품명)에는 영수증에 적힌 축약어, 불필요한 괄호, 기호(예: ')')를 제거하고, 오직 **순수 제품 본연의 명칭**만 기재하십시오 (화면 표시용이므로 상호명을 절대 포함시키지 마십시오).
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        contents: [
          {
            parts: [
              { text: "영수증을 분석하되, 품목별 [AI 복원] 영역에는 상호명을 제외한 순수 제품명만 복원하고, 최종 결제 총액(TOTAL)을 정확히 분석하여 지정된 양식으로 출력하시오." },
