...
- shopIndustry: 상호검색 결과를 바탕으로 해당 매장의 업종 및 업태를 파악하여 간략한 한 줄 요약과 함께 기재하십시오. (예: "음식점업 / 음식을 조리하여 제공하는 일반 음식점")
- [중복 계산 방지 규칙]: 세금 항목(부가세, 과세물품가액 등), 총합계, 받은금액, 거스름돈, 결제금액(신용카드 등 단순 결제수단 금액), 품목 할인의 단순 합산액은 절대 overallElements에 포함하지 마십시오.
- overallElements에는 품목별 할인이 아닌, 영수증 전체에 적용된 일괄 할인과 추가 비용(봉투값 등)만 포함하십시오. 영수증 전체 일괄 할인의 항목명은 반드시 "총액 차감 (할인명)" 형태로 구체적인 명칭을 병기하십시오.`;

    const promptText = `영수증 이미지를 분석하여 아래 포맷의 순수 JSON으로만 응답하시오:
{
  "shopOcr": "상호명",
  "shopName": "정식 상호명",
  "shopIndustry": "업종·업태 및 간략한 한줄요약 (상호검색 기반 발췌)",
  "date": "YYYY-MM-DD",
  "bizNo": "사업자번호",
  "phone": "전화번호",
  "address": "매장주소",
  "overallElements": [
    { "name": "항목명 (반드시 '총액 차감 (할인명칭)' 형태로 기재)", "amount": "할인은 -금액, 추가는 +금액" }
  ],
  "products": [
    {
