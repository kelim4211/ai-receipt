...
[상호명 [AI 복원] 및 스마트 검색 지능형 판단 규칙]
- 상호명 [OCR]이 '정보없음'이더라도, 고유 품번 패턴, 균일가 가격대, 품목 특징을 통해 특정 브랜드(예: 다이소 등)가 유추되는 경우 상호명 [AI 복원] 영역에 반영하십시오.
- **[지능형 검색 판단]**: 상호가 다이소 같은 자체 브랜드 유통사인지, 혹은 편의점·일반 소매점·식당 등 개별 가맹점/지점명인지 AI가 스스로 판단하십시오. 지점명이나 지역명이 포함된 편의점/일반 소매점의 경우 제품 검색 시 방해가 되므로, 검색용 키워드 조합 시 상호명을 완전히 배제하고 순수 제품 브랜드와 명칭 위주로만 복원하십시오.

[순수 상품명 추출 및 규격/수량 제거 절대 규칙]
- 복원제품명(두 번째 필드)은 영수증에 적힌 축약어, 불필요한 괄호, 기호(예: ')')를 제거하고, 실제 인터넷 쇼핑이나 제품 검색 시 가장 정확하게 매칭되는 정식 제조사 및 브랜드 명칭을 지능적으로 발췌하여 구성하십시오.
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        contents: [
          {
            parts: [
              { text: "영수증을 분석하되, 상호명이 없으면 상호명 [OCR]은 '정보없음'으로 처리하고, 편의점 등의 지점명은 제품 검색에서 자동 배제되도록 지능적으로 정식 명칭을 복원하며, 최종 결제 총액(TOTAL)을 정확히 분석하여 지정된 양식으로 출력하시오." },
