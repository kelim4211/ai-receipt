...
[규칙 1: 제품명 추출 및 정규화]
- 영수증의 축약어, 잘린 문자열, 품번을 추출하고, 내장된 유통 지식을 바탕으로 가장 타당한 표준 제품명(productAi)으로 완성하라. 가상의 제품명을 지어내지 말 것.

[규칙 2: 금액 추출 절대 원칙]
- 사칙연산, 나눗셈, 단가 역산 절대 금지.
- 'totalPrice'에는 영수증 행 우측 끝에 인쇄된 최종 금액 숫자를 있는 그대로 기입하라.`;

    const promptText = `영수증 이미지를 판독하여 아래 JSON 규격으로만 응답하라. 다른 설명이나 사족은 일체 포함하지 마라.
{
  "shopOcr": "영수증 상단 상호명",
  "shopName": "복원된 정식 상호명",
  ...
  "products": [
    ...
    // [방어 로직] 마크다운 코드블록(```json ... ```) 제거 및 앞뒤 공백 정리
    rawJsonText = rawJsonText.replace(/```json\n?|```/g, '').trim();

    // [방어 로직] 설명 문구가 섞여 있더라도 순수 최외곽 JSON 중괄호({ ... }) 구간만 발췌
