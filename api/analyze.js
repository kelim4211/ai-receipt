...
[제품검색 키워드 추출 엄격 규칙 (상호 결합 금지 원칙)]
1. 편의점 및 일반 소매점 배제: 영수증 상호가 편의점(CU, GS25 등), 일반 소매점, 마트, 식당 등의 개별 가맹점/지점명인 경우, 복원제품명(두 번째 필드)에 해당 상호나 지점명을 **절대 포함시키지 마십시오**. 오직 상품 본연의 명칭과 제조사/브랜드(예: 풀무원 등) 위주로만 구성하여 포털 제품 검색에 최적화시키십시오[cite: 3].
2. 유통사 브랜드 한정 허용: 영수증 상호가 '다이소' 등 자체 브랜드 유통사여서 상호명이 곧 제품 카테고리 검색에 결정적인 도움이 되는 경우에만 예외적으로 허용하십시오[cite: 4].
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        contents: [
          {
            parts: [
              { text: "영수증을 분석하되, 편의점 등의 지점명은 제품검색 키워드에 절대 포함시키지 말고 순수 제품명과 제조사 중심으로만 복원하며, 최종 결제 총액(TOTAL)을 정확히 분석하여 지정된 양식으로 출력하시오." },
