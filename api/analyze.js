...
- productAi(복원 제품명)는 절대 임의로 단어를 추가하거나 유추하지 말고, 원본(productOcr)에서 특수기호(*, -, 괄호 등)만 제거한 상태로 있는 그대로 발췌하십시오.`;
    ...
    const promptText = `영수증 이미지를 분석하여 아래 포맷의 순수 JSON으로만 응답하시오:
{
  ...
  "products": [
    ...
    try {
      ...
      console.warn('1차 JSON 파싱 실패, 자동 교정 시도...', rawJsonText);
      try {
